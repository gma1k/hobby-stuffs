...
# Ask the user if they want to see all or select countries
user_input = input("Do you want to see the weather for all countries or select some? (all/select): ")

# If the user wants to see all, loop through each country and each city and get the weather data
if user_input == "all":
...
# Loop for user selection
    ...
    # Ask the user to enter one or more countries separated by commas
    user_input = input(f"Which countries do you want to see? Choose from {countries} and separate by commas: ")
    # Split the user input by commas and strip any whitespace
    selected_countries = [country.strip() for country in user_input.split(",")]
    # Loop through each selected country and its cities and get the weather data
    for country in selected_countries:
        # Check if the country is valid, otherwise print an error message and skip it
...
ax.legend()
# Show or save (uncomment)the plot 
plt.show()
